  ...
  return (
    <>
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-['Bangers'] text-xl text-black dark:text-white">Popular Creators</h3>
        <Button
          variant="outline"
          size="sm"
          className="border border-black bg-white text-black hover:bg-gray-100 rounded-lg flex items-center gap-2 px-4 py-2"
        >
        ...
        {users.map((user, index) => (
          <Link to={`/profile/${user.id}`} key={user.id}>
            <motion.div
              ...
              className="flex items-center justify-between p-4 bg-white dark:bg-gray-900 rounded-lg border-2 border-black hover:shadow-lg transition-shadow"
            >
