...
export const FeedProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      // Generate fake data with comic theme
      const generatedUsers = generateUsers(15);
      setUsers(generatedUsers);

      // Set current user (Spider-Man/Peter Parker if available)
      const peterParker =
        generatedUsers.find((user) => user.username === "spiderman") ||
        generatedUsers[0];
      setCurrentUser(peterParker);

      // Generate comic-themed posts with the users
  ...
  return (
    <FeedContext.Provider
      value={{ posts, isLoading, likePost, bookmarkPost, currentUser }}
    >
      {children}
    </FeedContext.Provider>
  );
